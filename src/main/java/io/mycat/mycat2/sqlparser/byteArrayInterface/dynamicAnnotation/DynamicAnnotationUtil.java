        ...
        for (int i = 0; i < list.size(); i++) {
            for (int j = 0; j < list.size(); j++) {
                ...
                if (!one.equals(two)) {
                    ...
                    int k = 0;
                    int l = 0;
                    for (k = oneTokenList.length - 1; k < oneTokenList.length && k > -1 && l < twoTokenList.length; ) {
                        if (!oneTokenList[k].equals(twoTokenList[l])) {
                            k--;
                            continue;
                        } else {
                            break;
                    ...
                    k -= 1;
                    if (k > 0) {
                        String a = Stream.of(Arrays.copyOfRange(oneTokenList, oneTokenList.length - k + 1, oneTokenList.length)).collect(Collectors.joining(" "));
                        String b = Stream.of(twoTokenList).collect(Collectors.joining(" "));
                        if (isIn(a, b)) {
                            pos = k;
                        } else {
                            pos = k;
