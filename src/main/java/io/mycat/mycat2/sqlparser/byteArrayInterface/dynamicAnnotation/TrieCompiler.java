...
public class TrieCompiler {
    ...
    boolean isTrie;
    HashMap<Key, TrieCompiler> children = new HashMap<>();
    Set<String> callback;
    int backPos = 0;
    ...
    public String toCode2(boolean isRoot, TrieContext context) {
        context.x += 1;
        Map<Boolean, List<Map.Entry<Key, TrieCompiler>>> map = this.children.entrySet().stream().collect(Collectors.partitioningBy((k) -> k.getKey().getType() == QUESTION_MARK));
    ...
    public String toCode3(boolean isRoot, List<Map.Entry<Key, TrieCompiler>> entrySet, TrieContext context) {
        ...
        if (context.x % 3 == 0 || context.y % 5 == 0) {
            ...
            if (!"".equals(res.trim())) {
                ...
                context.funList.add("public  final int " + funName + "(int i, final int arrayCount, BufferSQLContext context, HashArray array, ByteArrayInterface sql){" + res + "return i;}");
    ...
    public String toCode4(boolean isRoot, List<Map.Entry<Key, TrieCompiler>> entrySet, TrieContext context) {
        StringBuilder stringBuilder = new StringBuilder();
        context.y += 1;
        for (Map.Entry<Key, TrieCompiler> i : entrySet) {
            context.index += 1;
            ...
            switch (type) {
                ...
                case QUESTION_MARK: {
                    q += "{int start=i;i=context.matchPlaceholders(i);";
                    break;
                }
                default: {
            ...
            if (callback != null && callback.size() != 0) {
                ...
                while (iterator.hasNext()) {
                    w += "context.setDynamicAnnotationResult(" + iterator.next() + ");";
                }
...
//                } else
                ...
                if (i.getValue().backPos != 0) {
                    w += "i=pick0(i-" + i.getValue().backPos +
            ...
            if (type == QUESTION_MARK) {
                String funName = context.genFun(entrySet.stream().map((s) -> Ascll.shiftAscll(s.getKey().getText(), false)).collect(Collectors.joining("_"))) + "_" + context.index;
                funName += "_quest";
                context.funList.add("public  final int " + funName + "(int i, final int arrayCount, BufferSQLContext context, HashArray array, ByteArrayInterface sql){" + stringBuilder.toString() + "return i;}");
